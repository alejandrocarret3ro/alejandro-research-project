...
class ModelTester:
    ...
    def denoise_frame(self, frames):
        """
        Denoise the center frame using temporal context from neighboring frames.
        
        Args:
            frames: List of PIL Images or numpy arrays (H, W, 3), length = num_input_frames.
                    Center frame is at index num_input_frames // 2.
        ...
        for frame in frames:
            if isinstance(frame, Image.Image):
                np_frame = np.array(frame, dtype=np.float32) / 255.0
                tensors.append(torch.from_numpy(np_frame).permute(2, 0, 1))
            else:
                # Assume numpy (H, W, 3) float32 [0, 1]
                tensors.append(torch.from_numpy(frame).permute(2, 0, 1))
        
        # Concatenate frames: (num_frames * 3, H, W)
        concat = torch.cat(tensors, dim=0)
        concat = concat.unsqueeze(0).to(self.device)  # (1, num_frames*3, H, W)
        
        with torch.no_grad():
            denoised = self.model(concat)
        
        denoised = denoised.squeeze(0).cpu()
        denoised = torch.clamp(denoised, 0.0, 1.0)
        denoised = denoised.permute(1, 2, 0).numpy().astype(np.float32)
    ...
    def denoise_noisy_video(self, video_folder_path, noise_std=50,
        ...
        for i in range(n):
            ...
            for offset in range(-self.half_window, self.half_window + 1):
                ...
                neighbor_frames.append(noisy_frames[idx])

            denoised = self.denoise_frame(neighbor_frames)
    ...
    def denoise_and_save_videos(self, video_folder_path, output_dir, noise_std=50,
        ...
        for i in range(n):
            ...
            for offset in range(-self.half_window, self.half_window + 1):
                idx = max(0, min(n - 1, i + offset))
                frame = self._resize_frame(noisy_frames[idx], resize_to)

...
class TemporalDenoiseDataset(Dataset):
    ...
    """
    def __init__(self, frame_data, psnr_range=(5, 45), resize_to=(256, 256),
                 use_fp16=True, num_input_frames=5, augment=False):
        """
        Args:
            ...
            num_input_frames: Number of temporal frames (3 or 5)
            augment: If True, apply random 90° rotations and horizontal flips
        """
    ...
    def __getitem__(self, idx):
        """
        Returns:
            noisy_sequence: (num_frames*3, H, W) tensor — N noisy RGB frames concatenated
        ...
        # Compute temporal neighbor indices with boundary handling
        half = self.num_input_frames // 2  # 1 for 3 frames, 2 for 5 frames
        neighbor_indices = []
        for offset in range(-half, half + 1):
            neighbor_pos = max(0, min(num_video_frames - 1, pos + offset))
            neighbor_indices.append(video_frames[neighbor_pos])

        # Load and resize all frames
        frames_np = [self._load_and_resize(frame_idx) for frame_idx in neighbor_indices]

        # Apply same augmentation to all frames (rotation + flip)
        if self.augment:
            rot_k = np.random.randint(0, 4)       # 0, 90, 180, 270 degrees
            do_flip = np.random.random() < 0.5     # horizontal flip
            for i in range(len(frames_np)):
                if rot_k > 0:
                    frames_np[i] = np.rot90(frames_np[i], k=rot_k).copy()
                if do_flip:
                    frames_np[i] = np.flip(frames_np[i], axis=1).copy()

        # Add independent noise to each frame (same noise level, different realizations)
        noisy_frames = []
        center_noise_std = None
        for i, frame_np in enumerate(frames_np):
            noisy, noise_std = self._add_noise(frame_np)
            noisy_frames.append(noisy)
            if i == self.num_input_frames // 2:  # center frame
                center_noise_std = noise_std

        # Clean center frame
        center_idx = self.num_input_frames // 2
        clean_curr_np = frames_np[center_idx]

        # Concatenate noisy frames: (H, W, num_frames*3) -> (num_frames*3, H, W)
        noisy_concat = np.concatenate(noisy_frames, axis=2)  # (H, W, N*3)
        noisy_tensor = torch.from_numpy(noisy_concat).permute(2, 0, 1) / 255.0

        clean_curr_tensor = torch.from_numpy(clean_curr_np).permute(2, 0, 1) / 255.0

        if self.use_fp16:
            noisy_tensor = noisy_tensor.half()
            clean_curr_tensor = clean_curr_tensor.half()

        return noisy_tensor, clean_curr_tensor, center_noise_std
...
def create_train_val_split(davis_root_dir, val_split=0.2, seed=42,
    ...
    Args:
        ...
        num_input_frames: Number of temporal frames per sample (3 or 5)
        augment: Enable data augmentation (rotation + flip) for training set
    ...
    # Create datasets — augmentation only for training, not validation
    train_dataset = TemporalDenoiseDataset(
        train_frames, psnr_range=psnr_range,
        resize_to=resize_to, use_fp16=use_fp16,
        num_input_frames=num_input_frames, augment=augment
    )
    val_dataset = TemporalDenoiseDataset(
        val_frames, psnr_range=psnr_range,
        resize_to=resize_to, use_fp16=use_fp16,
        num_input_frames=num_input_frames, augment=False  # never augment validation
    )
